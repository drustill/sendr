...
static void dump(const Node* node, int depth = 0) {
...
TEST_CASE("parses whitespace plaintext", "[HtmlParser]")
{
  std::string withWhitespace = "               whitespace               ";
  HtmlParser p(withWhitespace);

  std::unique_ptr<Node> root = p.Parse();

  REQUIRE(root->children.size() == 1);

  Node* node = root->children[0].get();
  CHECK(node->tag.empty());

  CHECK(node->text == "whitespace");
}

TEST_CASE("parses nest", "[HtmlParser]") {
  ...
  auto* ul = root->children[0].get();
  CHECK(ul->tag == "ul");
  REQUIRE(ul->children.size() == 2);

  CHECK(ul->children[0]->tag == "li");
  CHECK(ul->children[0]->children[0]->text == "one");

  CHECK(ul->children[1]->tag == "li");
  CHECK(ul->children[1]->children[0]->text == "two");
}

TEST_CASE("parses aarecord-list", "[HtmlParser]") {
